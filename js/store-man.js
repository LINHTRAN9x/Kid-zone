...
var app = angular.module('myAppp',[]);
    /* Get listProduct form localStorage */
    var jsonListProduct = localStorage.getItem('listProduct');
    var listProduct = JSON.parse(jsonListProduct);


    //Chuyen Object sang html
    var HTML = convertListObjectProductToHTML(listProduct);

    //link html vao container
    var nodeProduct = document.querySelector('.local-storage');
    nodeProduct.innerHTML = HTML;

    // function addToCart(id){
    //     alert('Add to cart voi sp co id la:' + id);
    // }
...
app.controller('myCtrl',function($scope){
    ...
    $scope.addToCart = function(idProduct) {
        ...
        for (var i=0; i < listItemCart.length; i++){
          var currentCart = listItemCart[i];

          if (currentCart.idProduct == idProduct) {
            listItemCart[i].qty ++;
            isListItemCart = true;
          }
        }

        if ( isListItemCart == false) {
            var itemCart = createCart(idProduct,1);
